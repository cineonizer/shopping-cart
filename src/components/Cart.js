...
const Cart = ({ cart, setCart }) => {
  ...
  const handleCheckoutBtnClick = () => {
    alert(
      'This is the end of my shopping cart project. Thanks for checking it out!'
    );
  };
  ...
  useEffect(() => {
    ...
    const sumOfPrices = cart.reduce((accumulator, objCart) => {
      return (
        accumulator +
        parseInt(objCart.price.replace(/,/g, '')) * parseInt(objCart.quantity)
      );
    }, 0);
